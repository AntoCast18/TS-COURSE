(()=>{
    ...
    let nombre: string;//declarar una variable y no asignarle un valor, agregar el tipo de dato

    let productPrice = 100;//forma inferida de declarar el tipo
    productPrice = 12;
    console.log('productPrice', productPrice);

    let customerAge: number = 28;//sin usar el motor de inferencia, se declara el tipo, forma explicita
    customerAge = customerAge + 1; //29 (?)
    console.log('customerAge', customerAge);

    let productInStock: number;//null, no esta inicializado y m marca una alerta al usar console.log e if
